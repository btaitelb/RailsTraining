require Rails.root.join('lib/weather_service')
...
class WeatherReadingsController < ApplicationController
  ...
  def index
    @weather_readings = WeatherReading.all.order(created_at: :desc)
  end
  ...
  def new
    @weather_reading = WeatherReading.new($weatherService.weather.to_hash)
  end
